// Logic agregasi makro (FRED + Yahoo) — dipakai bareng oleh /api/macro & /api/macro-narrative.
// Dipisah dari route biar narasi bisa manggil langsung (gak lewat HTTP internal yang kena proteksi Vercel).

const KEY = process.env.FRED_API_KEY

export type Obs = { date: string; value: number }
export type Pt = { d: string; v: number }
export type Packed = { value: number | null; prev: number | null; series: Pt[] }

export function hasFredKey() { return !!KEY }
...
export async function getMacroData() {
  if (!KEY) throw new Error('FRED_API_KEY belum di-set di environment.')
