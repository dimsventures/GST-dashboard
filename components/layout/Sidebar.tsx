...
export function Sidebar({ mobileOpen, onClose, collapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      {!collapsed && (
        <div className="sb-bottom">
          <button className="sb-cat" onClick={() => setCatOpen(true)} title="Pengingat: yang belum kelar" aria-label="Pengingat">
    ...
    {catOpen && (
      <div className="sb-pop-ov" onClick={e => { if (e.target === e.currentTarget) setCatOpen(false) }}>
        <div className="sb-pop">
          <div className="sb-pop-hd"><span>🐱 Yang masih nungguin lu{username ? ', ' + username : ''}…</span><button className="sb-pop-x" onClick={() => setCatOpen(false)}>✕</button></div>
          <div className="sb-pop-grid">
            {col('✦ Dream', '#ec4899', rem.dreams, 'Semua dream udah ke-realize 🎉')}
    ...
    <style>{`
      ...
      .sb-cat{position:absolute;top:-26px;left:50%;transform:translateX(-50%);z-index:0;background:none;border:none;padding:0;cursor:pointer;filter:drop-shadow(0 3px 7px rgba(0,0,0,.55));transition:transform .15s;}
      .sb-cat:hover{transform:translateX(-50%) translateY(-3px);}
      .sb-bottom-box{position:relative;z-index:1;background:#0c1428;box-shadow:0 -7px 20px rgba(0,0,0,.5);}
      .sb-pop-ov{position:fixed;inset:0;z-index:500;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.6);backdrop-filter:blur(3px);padding:24px;}
